...
try{
 for(const width of [1280,390]){
  ...
  await page.locator('[data-action="FUND"]').click();await done();await page.locator('#policy-form button').click();await expect(page.locator('#policy-status')).toContainText('ACTIVE');await page.locator('[data-action="ENTER"]').click();await done();assert.equal((await state()).deployed,80000);
